def save_historical_event_collection(element, world):
    ...
    exclude_tags = ['start_seconds72', 'end_seconds72', 'ordinal', 'feature_layer_id', 'attacking_squad_entity_pop', 'defending_squad_entity_pop', 'individual_merc', 'attacking_merc_enid', 'company_merc']

    chronicle_id, aggressor_entity_id, attacking_squad_animated, attacking_squad_site, defender_entity_id, defending_squad_animated, defending_squad_site, civ_id, coords, end_year, name, occasion_id, outcome, site_id, start_year, subregion_id, type, attacking_squad_race, defending_squad_race, target_entity_id, war_event_collection, parent_event_collection = None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None
    ...
    for child in element:
        ...
        if child.tag == 'id':
            ...
            type = child.text
        elif tag == 'war_eventcol':
            if int(child.text) > 0:
                war_event_collection = child.text
        elif tag == 'parent_eventcol':
            if int(child.text) > 0:
                parent_event_collection = child.text
        elif tag == 'eventcol':
            if int(child.text) > 0:
                event_collections.append(child.text)
    ...
    if civ_id:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'civ_id': civ_id})
    if site_id:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'site_id': site_id})
    if aggressor_entity_id:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'aggressor_entity_id': aggressor_entity_id})
    if defender_entity_id:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'defender_entity_id': defender_entity_id})
    if attacking_squad_site:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'attacking_squad_site': attacking_squad_site})
    if defending_squad_site:
        try:
            ...
            missing.append({'event_collection': event_collection.id, 'defending_squad_site': defending_squad_site})
    if len(attacking_hfids) > 0:
        for hf_id in attacking_hfids:
            try:
                ...
                missing.append({'event_collection': event_collection.id, 'attacking_hfid': hf_id})
    if len(defending_hfids) > 0:
        for hf_id in defending_hfids:
            try:
                ...
                missing.append({'event_collection': event_collection.id, 'defending_hfid': hf_id})
    if len(noncom_hfids) > 0:
        for hf_id in noncom_hfids:
            try:
                ...
                missing.append({'event_collection': event_collection.id, 'noncom_hfid': hf_id})
    if len(events) > 0:
        for event in events:
            try:
                ...
                missing.append({'event_collection': event_collection.id, 'event': int(event)})
    if target_entity_id:
        try:
            entity = models.Entities.objects.get(world=world, chronicle_id=target_entity_id)
            event_collection.target_entity_id = entity
        except models.Entities.DoesNotExist:
            missing.append({'event_collection': event_collection.id, 'target_entity_id': target_entity_id})
    if war_event_collection:
        try:
            war_event_collection = models.HistoricalEventCollections.objects.get(world=world, chronicle_id=war_event_collection)
            event_collection.war_event_collection = war_event_collection
        except models.WarEventCollections.DoesNotExist:
            missing.append({'event_collection': event_collection.id, 'war_event_collection': war_event_collection})
    if parent_event_collection:
        try:
            parent_event_collection = models.HistoricalEventCollections.objects.get(world=world, chronicle_id=parent_event_collection)
            event_collection.parent_event_collection = parent_event_collection
        except models.HistoricalEventCollections.DoesNotExist:
            missing.append({'event_collection': event_collection.id, 'parent_event_collection': parent_event_collection})
    if len(event_collections) > 0:
        for event_collection_id in event_collections:
            try:
                event_collection = models.HistoricalEventCollections.objects.get(world=world, chronicle_id=event_collection_id)
                event_collection.event_collection.add(event_collection)
            except models.HistoricalEventCollections.DoesNotExist:
                missing.append({'event_collection': event_collection.id, 'child_event_collection': event_collection_id})
